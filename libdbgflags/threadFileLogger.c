...
static inline int createFile(ThreadFileData *threadFileData) {
	...
	threadFileData->logFile = open(threadFileData->fullFileName,flags,S_IRUSR|S_IWUSR|S_IRGRP);
				...
				if (likely(EXIT_SUCCESS == error)) {
					...
					if (written > 0) {
						...
						if (LOG_PRI(priority) <= LOG_ERR) {
							...
							if (error != 0) {
								error = errno;
								ERROR_MSG("fdatasync to %s error %d (%m)",threadFileData->fullFileName,error);
							}
							error = posix_fadvise(threadFileData->logFile, 0,0,POSIX_FADV_DONTNEED); /* tell the OS that log message bytes could be released from the file system cache */
							if (error != 0) {
