...
static inline void cmdLineHelp(void) {
    const char *loggersParameters = openLoggerFromCmdLineHelp();
//DYNAMIC_LOGGER_OPT_LOG_PARAMETERS
}

static inline int parseCmdLineParameters(int argc, char *argv[],dynamicLoggerConfiguration *configuration, int *flags) {
...
#define displayLevel(stream,level,mask,string) if (level == (LOG_PRI(mask) & level)) fprintf(stream,string " ")
static __inline void displayLogMask(FILE *stream,const int syslogMask) {
    int a = (LOG_PRI(syslogMask) & LOG_CRIT);
    printf("%d\t0x%X\n",a,syslogMask);
    fprintf(stream,"log level set:");
    //if (LOG_INFO == (LOG_PRI(syslogMask) & LOG_INFO)) fprintf(stream,string " ")
...
int main(int argc, char *argv[]) {
    ...
    displayLogMask(stdout,0x7F);
    displayLogMask(stdout,0x6F);
    displayLogMask(stdout,0x5F);
    displayLogMask(stdout,0x4F);
    displayLogMask(stdout,0x3F);
    displayLogMask(stdout,0x2F);
    displayLogMask(stdout,0x0F);
    displayLogMask(stdout,0x1F);
return 0;
